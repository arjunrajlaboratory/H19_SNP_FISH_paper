function [] = extractSpotCSVs_gfp_cellArrayObj(outDir)
%Extracts Spot CSVs including the GFP channel (not fitted)
...
while(tools.iterator.continueIteration)
    ...
    if ~isempty(regexp(tools.objectHandle.getLabelsOfNodesWithData, 'gfp:Spots'))
    numGFP = tools.objectHandle.getData('gfp:Spots').getNumSpots;
    alexa.numGFP = ones(height(alexa),1) * numGFP;
    end
    ...
    if ~isempty(regexp(tools.objectHandle.getLabelsOfNodesWithData, 'gfp:Spots'))
    numGFP = tools.objectHandle.getData('gfp:Spots').getNumSpots;
    alexa2.numGFP = ones(height(alexa2),1) * numGFP;
    end
...
cellArrayObj_toWrite = array2table(cellArrayObj);
cellArrayObj_toWrite.Properties.VariableNames = {'cellID', 'ArrayNum', 'ObjNum'};

writetable(cellArrayObj_toWrite,[expName '_CellArrayObj.csv'])
