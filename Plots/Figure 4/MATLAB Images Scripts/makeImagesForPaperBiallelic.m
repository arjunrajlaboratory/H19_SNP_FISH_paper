%% Read the data files
cd 3-25-2015_Bi_6-1_1x
cy    = readmm('cy012.tif');
tmr   = readmm('tmr012.tif');
dapi  = readmm('dapi012.tif');
alexa = readmm('alexa012.tif');
gfp   = readmm('gfp012.tif');
...
RGB = cat(3,cropCyScaled*orange(1), cropCyScaled*orange(2), cropCyScaled*orange(3) + cropDapiScaled);
imshow(RGB);
RGB = im2uint16(RGB);
imwrite(RGB,'bialleliccyImage.tif');

RGB = cat(3,cropTmrScaled*cyan(1), cropTmrScaled*cyan(2), cropTmrScaled*cyan(3) + cropDapiScaled);
imshow(RGB)
RGB = im2uint16(RGB);
imwrite(RGB,'biallelictmrImage.tif');

RGB = cat(3,cropAlexaScaled, cropAlexaScaled, cropAlexaScaled + cropDapiScaled);
imshow(RGB)
RGB = im2uint16(RGB);
imwrite(RGB,'biallelicalexaImage.tif');
